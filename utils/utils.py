...
def rho_select(pad, lengths):
    ...
    idx_ = (lengths-1).view(-1, 1).expand(
                pad.size(0), pad.size(2)).unsqueeze(1)
    ...
    if torch.is_tensor(batch[0]):
        ...
        if 1 < batch[0].dim():  # image features
            ...
            if _use_shared_memory:
                ...
                if batch[0].size(-1) != batch[0].size(-2):  # bottom-up-feature
                    numel = len(batch) * max_num_boxes * batch[0].size(-1)
                    storage = batch[0].storage()._new_shared(numel)
                    out = batch[0].new(storage).reshape(len(batch), max_num_boxes, batch[0].size(-1))
                elif batch[0].size(-1) == 512 or batch[0].size(-1) == 224:
                    numel = len(batch) * max_num_boxes * batch[0].size(-1) * batch[0].size(-2)
                    storage = batch[0].storage()._new_shared(numel)
                    out = batch[0].new(storage).reshape(len(batch), max_num_boxes, batch[0].size(-1), batch[0].size(-2))
                else:  # adj_matrix
                    numel = len(batch) * max_num_boxes * max_num_boxes
                    storage = batch[0].storage()._new_shared(numel)
                    out = batch[0].new(storage).reshape(len(batch), max_num_boxes, max_num_boxes)
            if batch[0].size(-1) != batch[0].size(-2):  # bottom-up-feature
                # warning: F.pad returns Variable!
                return torch.stack(
                    [F.pad(x,
                           (0, 0, 0,
                            max_num_boxes-x.size(0))).data for x in batch],
                    0, out=out)
            else:  # adj_matrix
                return torch.stack(
                    [F.pad(x,
                           (0, max_num_boxes-x.size(0),
                            0, max_num_boxes-x.size(0))).data for x in batch],
                    0, out=out)
        else:
            if _use_shared_memory:
                # If we're in a background process, concatenate directly into a
                ...
                storage = batch[0].storage()._new_shared(numel)
                out = batch[0].new(storage).reshape(len(batch), *batch[0].size())
        ...
        return {key: default_collate(
                        [d[key] for d in batch]) for key in batch[0]}

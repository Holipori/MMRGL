    ...
    def forward(self, v_relation, q_emb, b):
        ...
        for g in range(self.glimpse):
            ...
            if self.use_counter:
                ...
                q_emb = q_emb + self.c_prj[g](embed.float()).unsqueeze(1)
...
class BUTD(nn.Module):
    ...
    def logits(self, v, q):
        ...
        # print('joint_repr', joint_repr.isnan().any())
        # print('logits', logits.isnan().any())
        # if logits.isnan().any():
        #     print(joint_repr[0])

        # logits = torch.mean(joint_repr,-1).unsqueeze(-1)
        # logits = nn.functional.softmax(torch.mean(joint_repr,-1)).unsqueeze(-1)
        return logits
